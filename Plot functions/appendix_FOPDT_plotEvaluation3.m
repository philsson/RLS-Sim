...
if plot_z_axis
    ...
    length3 = length(xLength);
        ...
        figure('Name', 'Z-axis FOPDT Evalution 3') %, 'Position', [110, 800, 1290,320]); clf;
...
%          saveas(gcf,'Results\Z-axis\Evaluation 3\EVO and Manual Tuning\3EVOManualtuningZ','svg');
        ...
        if get_easy_files
           saveas(gcf,'Results\3EVOFOPDTZ','epsc');
        end
        ...
        axis([0 max(xLength(1:l1:end)) 0 75])
        h = ylabel('MISE/sp');
        set(h,'FontSize',textSize+textSize_offset_y);
        set(gca,'XTickLabel','');
        ...
        if get_easy_files
           saveas(gcf,'Results\3EVOFOPDT_errorsZ','epsc');
        end
        ...
        if get_easy_files
           saveas(gcf,'Results\3EVOFOPDTZ_inertiaZ','epsc');
        end
